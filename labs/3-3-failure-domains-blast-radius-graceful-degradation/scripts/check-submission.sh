...
# docs/review.md exists. Ported from lab 3-2 to keep the same rubric
# discipline ("every claim cites an artifact").
...
# Read candidates into a newline-delimited string so this works on
# macOS bash 3.x (no mapfile/readarray). Each candidate is processed
...
FAILURE_DOMAINS="docs/failure-domains.md"
if [[ ! -f "${FAILURE_DOMAINS}" ]]; then
  echo "WARN: ${FAILURE_DOMAINS} not filled in - step 3 asks for the dep classification."
fi
